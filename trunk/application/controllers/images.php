...
class Images extends MY_Controller {
  ...
  function ajax_list(){
    ...
    $result["images"] = $this->imagesModel->get($_post);
  ...
  function ajax_upload(){
    ...
    $dir_1 = Hash::make("post_images")->hash($_post["name"]);

    if(!file_exists($dir_1)){
      mkdir($dir_1, 0755,true);
    }
    ...
    if (!$chunks || $chunk == $chunks - 1) {
      ...
      $result = $this->imagesModel->addRecord($Obj);
      $string ="";
      foreach($_post AS $key=>$value){
        $string .= ' , "'.$key.'" : "'.$value.'"';
      }
      if($result){
        die('{"jsonrpc" : "2.0", "result" : "TRUE", "id" : "'.$result.'"'.$string.'}');
      }else{
        die('{"jsonrpc" : "2.0", "result" : "FALSE", "id" : "0"'.$string.'}');
      }
